...
# ... [Keep your CONFIG and BATTLE_DICTIONARY exactly as they are] ...

console = Console()

def generate_dashboard(last_msg=None, last_user=None, verbs=None, status="IDLE"):
    """Creates a visual 'landing page' in the terminal."""
    table = Table(title="📡 ABM Tactical Translator - Live Monitor", expand=True)
    ...
    if last_msg:
        v_str = f"{verbs[0]} | {verbs[1]} | {verbs[2]}" if verbs else "WAITING..."
        table.add_row(
...
def start_translator():
    # Load context once
    context_preview = f"Tactical Context Loaded ({len(CSV_CONTEXT)} chars)"
    ...
    # This creates the 'Live' updating view
                    ...
                    if " PRIVMSG " in line:
                        ...
                        if len(parts) >= 3:
                            ...
                            if is_clean(msg_content):
                                ...
                                live.update(generate_dashboard(msg_content, user, None, "THINKING..."))
                                
                                # Process
                                v1, v2, v3 = get_verbs_from_lm_studio(msg_content)
                                tactical_json = build_tactical_json(msg_content, v1, v2, v3)
                                
                                # Update UI with results
                                live.update(generate_dashboard(msg_content, user, (v1, v2, v3), "COMPLETED"))
                                
                                # Log the JSON to a hidden file instead of cluttering the screen
                                with open("tactical_output.log", "a") as f:
                                    f.write(json.dumps(tactical_json) + "\n")
                            else:
                                live.update(generate_dashboard(msg_content, user, ("-", "-", "-"), "FILTERED (NOISE)"))
            except Exception as e:
                live.update(Panel(f"[bold red]CRITICAL ERROR:[/bold red] {e}"))
                break
